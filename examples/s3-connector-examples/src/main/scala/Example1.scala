...
object Example1 extends ZIOAppDefault {
  ...
  val program: ZIO[S3Connector, Object, String] = {
    for {
      ...
      _                <- ZStream(bucket1, bucket2).run(createBucket(region))
      buckets          <- listBuckets(region).runCollect
      objectKey         = ObjectKey("quote.txt")
      _                <- ZStream.fromIterable(quote.getBytes(StandardCharsets.UTF_8)).run(putObject(bucket1, objectKey, region))
      _                <- ZStream(CopyObject(bucket1, objectKey, bucket2)).run(copyObject)
      objectsPerBucket <- ZIO.foreach(buckets)(bucket => listObjects(bucket, region).runCollect.map((bucket, _)))
      _ <- ZIO.foreach(objectsPerBucket) { case (bucket, objects) =>
             Console.printLine(s"Objects in bucket $bucket: ${objects.mkString}")
           }
      text            <- getObject(bucket2, objectKey, region) >>> ZPipeline.utf8Decode >>> ZSink.mkString
      _               <- ZIO.foreachPar(buckets)(bucket => ZStream(objectKey).run(deleteObjects(bucket, region)))
      bucketsNonEmpty <- ZIO.foreachPar(buckets)(bucket => ZStream(objectKey).run(existsObject(bucket, region)))
      _ <- ZStream
             .fromChunk(buckets)
             .run(deleteEmptyBucket(region))

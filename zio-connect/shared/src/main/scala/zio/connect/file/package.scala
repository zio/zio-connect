...
package object file {
  ...
  type FileConnector = FileConnector.Service
  ...
  object FileConnector {
    ...
    object Service {
      val live: Service = new Service {
        ...
        def pollUpdates(file: Path, queue: Queue[Byte], ref: Ref[Long]): ZIO[Any, IOException, Unit] =
          for {
            cursor <- ref.get
        ...
        def watchUpdates(file: Path, watchService: WatchService, queue: Queue[Byte], ref: Ref[Long]): ZIO[Any, IOException, Unit] =
          for {
            cursor <- ref.get
    ...
    val any: ZLayer[FileConnector, Nothing, FileConnector] = ZLayer.service[FileConnector]
    // Do we need this?
